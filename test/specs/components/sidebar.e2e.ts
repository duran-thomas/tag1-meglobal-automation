...
describe("Sidebar Component Tests", () => {
    ...
    it("[S3C1025] Verify that a site content-admin can create a sidebar component", async () => {
        ...
        await (
            await QALayoutPage.btnSidebarNav
        ...
        await (
            await SidebarBlockPage.sidebarElement
        ).scrollIntoView({ behavior: "auto", block: "center" });
    });
    ...
    it("[S3C1026] Verify sidebar section configurations", async () => {
        ...
        await expect(SidebarBlockPage.inputFirstColumnClasses).toHaveValue(
            "col-span-full lg:col-span-6"
        );
        await expect(SidebarBlockPage.inputSecondColumnClasses).toHaveValue(
            "col-span-full lg:col-start-8 lg:col-span-17"
        );
    });
    ...
    it("[S3C968] Verify that Group Content Menu links are editable", async () => {
        ...
        await SidebarBlockPage.createEditedNodeLayoutPage(
            sidebarBlockData.nodeText
        );
    ...
    */
    ...
    it("[S3C1028] Verify that each menu list item should be an active link to their respective resources", async () => {
        ...
        if (process.env.ENV === "dev") {
            await expect(await browser.getUrl()).toBe(
                "https://dev-content.montefioreeinstein.org/research/clinical-trials"
            );
        } else {
            await expect(await browser.getUrl()).toBe(
    ...
    it("[S3C1993] Verify analytics for Sidebar Component", async () => {
        await SidebarBlockPage.setupSidebar(sidebarBlockData.title);
        await expect(SidebarBlockPage.successMsg).toBeDisplayed();
        ...
        await browser.execute(() => {
            const menuItem = document.querySelectorAll(
                '.mf-sidebar__item a[data-analytics-click-text="Training Program Contact"]'
            );
            menuItem[1].setAttribute("target", "_blank");
        });
        ...
        const expectedAnalyticsData = [
            {
                clickText: "Training Program Contact",
        ...
        fs.writeFile(
            "analyticsTestEvidence/sidebar.json",
