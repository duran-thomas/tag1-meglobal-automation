...
describe('Quick Actions Component Tests', () => {
    ...
    it('[S3C924] Verify that a Content Administrator can create a Quick Actions menu component with an external link', async () => {
        ...
        await (await QuickActionsBlockPage.quickActionsElement).scrollIntoView({ block: 'start' });

        await expect(await QuickActionsBlockPage.quickActionsElement).toBeDisplayedInViewport();
        ...
        const buttonHref = await QuickActionsBlockPage.quickActionsButton.getAttribute('href');
