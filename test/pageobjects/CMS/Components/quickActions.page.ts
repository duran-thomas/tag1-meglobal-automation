...
class QuickActionsBlockPage extends Page {
    ...
    public async createMenu(title: string) {
      (await this.btnAddNewMenu).click();
      await this.linkGroupQuickActions.waitForDisplayed({timeout:2000});
      (await this.linkGroupQuickActions).click();
      (await this.inputTitle).waitForDisplayed({timeout:2000});
    ...
    public async addLinkToMenu(menuTitle:string, link: string) {
        (await this.createdMenu).waitForDisplayed({timeout:2000});
        (await this.createdMenu).click();
        (await this.addLink).click();
        (await this.inputMenuLinkTitle).waitForDisplayed({timeout:2000});
        (await this.inputMenuLinkTitle).setValue(menuTitle);
    ...
    public async createNode(title: string) {
        (await this.btnAddNewContent).waitForDisplayed({timeout:2000});
        (await this.btnAddNewContent).click();
        await this.linkGroupLayoutPage.waitForDisplayed({timeout:2000});
        (await this.linkGroupLayoutPage).click();
